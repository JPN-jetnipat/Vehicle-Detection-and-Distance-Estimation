...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(
        ...
        # Update labels
        include_class = include_classes or []  # filter labels to include only these classes (optional)
        self.segments = list(self.segments)
        include_class_array = np.array(include_class).reshape(1, -1) if len(include_class) else np.empty((1, 0))
