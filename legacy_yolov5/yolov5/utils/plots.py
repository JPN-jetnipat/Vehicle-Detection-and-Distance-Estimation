...
def output_to_target(output, distances=None, max_det=300):
    """Converts YOLOv5 model output to plotting targets and optionally appends distance per detection."""
    targets = []
    for i, o in enumerate(output):
        box, conf, cls = o[:max_det, :6].cpu().split((4, 1, 1), 1)
        j = torch.full((conf.shape[0], 1), i)
        if distances is not None and i < len(distances):
            dist = [float("nan") if d is None else float(d) for d in distances[i][: box.shape[0]]]
            if isinstance(dist, torch.Tensor):
                dist = dist[: box.shape[0]].view(-1, 1).cpu().float()
            else:
                dist = torch.as_tensor(dist, dtype=torch.float32).view(-1, 1)
        else:
            dist = torch.full((conf.shape[0], 1), float("nan"))
        targets.append(torch.cat((j, cls, xyxy2xywh(box), conf, dist), 1))
    return torch.cat(targets, 0).numpy()


@threaded
def plot_images(images, targets, paths=None, fname="images.jpg", names=None, image_texts=None):
    ...
    for i in range(bs):
        ...
        if image_texts and i < len(image_texts) and image_texts[i]:
            annotator.text([x + 5, y + 24], text=str(image_texts[i])[:40], txt_color=(220, 220, 220))
        if len(targets) > 0:
            ...
            distance = ti[:, 7] if ti.shape[1] > 7 else None
            ...
            for j, box in enumerate(boxes.T.tolist()):
                ...
                if labels or conf[j] > 0.25:  # 0.25 conf thresh
                    if distance is not None and np.isfinite(distance[j]):
                        label = f"{cls} {distance[j]:.1f}m" if labels else f"{cls} {conf[j]:.1f} {distance[j]:.1f}m"
                    else:
