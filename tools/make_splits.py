...
def write_lists(names, stem, split_dir_repo, split_dir_yolo, img_subdir):
    ...
    # repo-root-relative on purpose (NOT .resolve()'d to an absolute path):
    # this project always runs commands from the repo root (every RUNBOOK
    # command does), and ultralytics' get_img_files() uses a non-"./" line
    # in one of these list files as-is, resolved against the process cwd at
    # load time. An absolute path baked in here would instead hardcode
    # wherever *this script* happened to be run from - which silently broke
    # once already (2026-08-16: a prior run baked in an ephemeral sandbox
    # session's mount path, which stopped existing the moment that session
    # ended). Relative-from-repo-root is the only form that's valid both on
    # this Mac and on the GPU server after a git pull.
    images_root = Path(split_dir_yolo).parent / "images"
