...
Ported unchanged from the old project - already class-list-agnostic (reads
`names:` from a YOLO data yaml), so swapping in bdd100k_vehicle5.yaml just
works with no code change.

Categories present in the JSON but absent from `names` are skipped (counted
in the stats) - this is how the 5-class filter (car/truck/bus/motor/bike) is
actually applied: everything else (person, rider, traffic light, traffic
sign, train) silently becomes a "skipped category" box.

Images with zero kept boxes still get an EMPTY .txt file (YOLO treats them
as background images) so every arm sees identical data - confirmed default:
keep, don't drop, per project decision (2026-08-14).

Convert the FULL train.json and FULL val.json each exactly once (this
script doesn't know or care about train/val/test splits, or about the
undefined-weather/timeofday cleaning). tools/make_splits.py handles all of
that afterwards purely by writing out lists of filenames - a converted
label file for an image that ends up excluded from every split just sits
unused in dataset/yolo/labels/, same as the old project's convention.
...
unsafe on a shared/limited-RAM server. We therefore stream entries one at a
time with ijson (in requirements.txt) - peak RAM stays ~tens of MB. Falls
back to json.load (with a loud warning) only if ijson is missing.
...
def load_names(data_config: str):
    ...
    names = cfg["names"]
    if isinstance(names, dict):  # {0: car, 1: truck, ...}
