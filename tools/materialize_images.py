...
Symlinks are written as RELATIVE paths (relative to --out-dir), not
absolute ones. Absolute symlinks baked in from inside a container/sandbox
bind-mount bake in that session's mount path, which stops resolving the
moment the session ends - this bit us once already (2026-08-16 folder
migration). Relative links stay valid no matter where this project folder
is mounted, copied, or moved.
...
def main():
    ...
    for f in src_root.rglob(f"*{args.ext}"):
        ...
        if link.exists() or link.is_symlink():
            n_exists += 1
            continue
        rel_target = os.path.relpath(f, out_resolved)
