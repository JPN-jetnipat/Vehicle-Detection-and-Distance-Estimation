...
def train(hyp, opt, device, callbacks):
    ...
    # Determine include_classes indices if dataset specifies names to include
    include_classes = None
    try:
        if data_dict and data_dict.get("include"):
            # data_dict['names'] is dict[int]=name
            name_to_idx = {v: k for k, v in data_dict["names"].items()}
            include_classes = [name_to_idx[n] for n in data_dict.get("include") if n in name_to_idx]
    except Exception:
        include_classes = None
    ...
    if RANK in {-1, 0}:
        epoch_iter = tqdm(epoch_iter, total=epochs - start_epoch, desc="Epochs", bar_format=TQDM_BAR_FORMAT)
    for epoch in epoch_iter:  # epoch ------------------------------------------------------------------
        ...
        if RANK in {-1, 0}:
            ...
            if not noval or final_epoch:  # Calculate mAP
                results, maps, _, distance_metrics = validate.run(
                    data_dict,
                ...
                if distance_metrics:
                    LOGGER.info(
                        f"Distance estimation MAE: {distance_metrics['mae']:.3f} m, "
                        f"RMSE: {distance_metrics['rmse']:.3f} m"
                    )
        ...
        if RANK in {-1, 0}:
            epoch_iter.set_postfix_str(f"{time.time() - epoch_start_time:.1f}s")
        if stop:
    ...
    if RANK in {-1, 0}:
        ...
        for f in last, best:
            if f.exists():
                ...
                if f is best:
                    LOGGER.info(f"\nValidating {f}...")
                    results, _, _, distance_metrics = validate.run(
                        data_dict,
                        batch_size=batch_size // WORLD_SIZE * 2,
                        imgsz=imgsz,
