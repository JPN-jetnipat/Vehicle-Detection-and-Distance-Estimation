...
# Ensure both the `yolov5` package folder and the repository root are on sys.path
PROJECT_ROOT = FILE.parents[1]
if str(ROOT) not in sys.path:
    sys.path.append(str(ROOT))  # add yolov5 folder to PATH
if str(PROJECT_ROOT) not in sys.path:
    sys.path.append(str(PROJECT_ROOT))  # add repo root so sibling packages (e.g. geometry) import
...
from geometry.distance_estimator import estimate_distance


def save_one_txt(predn, save_conf, shape, file, distances=None):
    ...
    Args:
        ...
        distances (list[float] | None): Optional per-prediction estimated distances in meters.
    ...
    Examples:
        ...
        save_one_txt(predn, save_conf=True, shape=(640, 480), file="output.txt", distances=[12.4])
        ```
    ...
    for i, (*xyxy, conf, cls) in enumerate(predn.tolist()):
        xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()  # normalized xywh
        if distances is not None:
            distance = distances[i] if i < len(distances) and distances[i] is not None else float("nan")
            line = (cls, *xywh, conf, distance) if save_conf else (cls, *xywh, distance)  # label format
        else:
...
def run(
    ...
    if not training:
        ...
        # Determine include_classes if dataset specifies inclusion list
        include_classes = None
        try:
            if data.get("include"):
                name_to_idx = {v: k for k, v in data["names"].items()}
                include_classes = [name_to_idx[n] for n in data.get("include") if n in name_to_idx]
        except Exception:
            include_classes = None
    ...
    # For optional distance MAE/RMSE computation
    compute_distance_metrics = bool(data.get("distance_csv") and data.get("focal_length_px") and data.get("real_widths"))
    gt_distances_map = {}
    pred_distances_list = []
    gt_distances_list = []
    ...
    if compute_distance_metrics:
        ...
        try:
            with open(data["distance_csv"], newline="") as csvfile:
                reader = csv.DictReader(csvfile)
                for row in reader:
                    # Expect column 'Image Name' or 'image' or 'name' and 'Distance'
                    key = row.get("Image Name") or row.get("image") or row.get("name")
                    if key:
                        gt_distances_map[Path(key).stem] = float(row.get("Distance") or row.get("distance") or row.get("Distance_m") or 0.0)
        except Exception:
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        for si, pred in enumerate(preds):
            ...
            # Optional: estimate distances per image and compare to ground truth if provided
            pred_distances = None
            if compute_distance_metrics:
                # predn is in native space after scale_boxes
                widths_px = (predn[:, 2] - predn[:, 0]).cpu().numpy() if predn.numel() else np.array([])
                pred_distances = []
                distances_pred = []
                for k in range(len(predn)):
                    cls = int(predn[k, 5])
                    name = names[cls]
                    real_width_m = data["real_widths"].get(name) if isinstance(data.get("real_widths"), dict) else None
                    if real_width_m is None:
                        pred_distances.append(None)
                        continue
                    focal_px = float(data["focal_length_px"])
                    w_px = float(widths_px[k])
                    if w_px <= 0:
                        pred_distances.append(None)
                        continue
                    d = estimate_distance(focal_px, float(real_width_m), w_px)
                    if d is None:
                        pred_distances.append(None)
                        continue
                    pred_distances.append(float(d))
                    distances_pred.append(d)
                while len(pred_distances) < len(predn):
                    pred_distances.append(None)
                if distances_pred:
                    mean_pred = float(np.mean(distances_pred))
                    image_id = Path(path).stem if path.stem.isnumeric() else Path(path).stem
                    gt = gt_distances_map.get(image_id)
                    if gt is not None:
                        pred_distances_list.append(mean_pred)
                        gt_distances_list.append(gt)
    ...
    for i, c in enumerate(ap_class):
        maps[c] = ap[i]
    # Optional: compute MAE and RMSE for distance estimation
    try:
        if compute_distance_metrics and len(gt_distances_list):
            gt_arr = np.array(gt_distances_list)
            pred_arr = np.array(pred_distances_list)
            mae_val = float(np.mean(np.abs(gt_arr - pred_arr)))
            rmse_val = float(np.sqrt(np.mean((gt_arr - pred_arr) ** 2)))
            distance_metrics = {"mae": mae_val, "rmse": rmse_val}
            LOGGER.info(f"Distance estimation MAE: {mae_val:.3f} m, RMSE: {rmse_val:.3f} m")
    except Exception:
        pass

    return (mp, mr, map50, map, *(loss.cpu() / len(dataloader)).tolist()), maps, t, distance_metrics
